...
Deno.serve(async () => {
  ...
  // Get today's snapshots for current-day alerts
  ...
  // Get tomorrow's snapshots for pre-medication alerts
  const { data: tomorrowSnapshots } = await supabase
    .from('pollen_snapshots')
    .select('user_id, snapshot')
    .in('user_id', allAlertIds)
    .eq('date', tomorrowStr);
  ...
  // Pre-medication alert: send at ~8 PM if tomorrow will be High (score >= 65)
  const currentHourUTC = new Date().getUTCHours();
  if (currentHourUTC >= 19 && currentHourUTC <= 21) {
    for (const row of (tomorrowSnapshots ?? [])) {
      const uid = row.user_id as string;
      const snap = row.snapshot as Record<string, number>;
      const score = pollenScore(snap);
      if (score >= 65 && highRiskIds.includes(uid)) {
        preMedTargets.push(uid);
      }
    }
  }
  ...
  if (preMedTargets.length) {
    const { data: subs } = await supabase
      .from('push_subscriptions')
      .select('endpoint, p256dh, auth')
      .in('user_id', preMedTargets);
    if (subs?.length) {
      const results = await sendPush(subs, {
        title: '💊 High Pollen Tomorrow',
        body: 'Tomorrow looks high. If you take antihistamines, tonight is the ideal time to dose.',
        url: '/',
      });
      sent += results.filter(r => r.status === 'fulfilled').length;
    }
  ...
  return new Response(
    JSON.stringify({ sent, highRisk: highRiskTargets.length, clearSkies: clearSkiesTargets.length, preMed: preMedTargets.length }),
