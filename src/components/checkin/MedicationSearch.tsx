...
export function MedicationSearch({ medications, onChange }: Props) {
  ...
  return (
    <div>
      <label className="block text-sm font-medium text-gray-700 mb-2">Medication taken today</label>

      {medications.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-2">
          {medications.map((m, i) => (
            <div key={i} className="flex items-center gap-1 bg-sky-50 border border-sky-200 rounded-full px-3 py-1 text-xs text-sky-800">
      ...
      <div className="relative">
        <input
          ...
          className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-sky-pilot"
        />
          ...
          <div className="absolute z-20 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden">
            {results.map((r, i) => (
              <button
                ...
                type="button"
                onClick={() => addMed(r)}
                className="w-full text-left px-4 py-2.5 hover:bg-sky-50 flex items-center justify-between gap-2 border-b border-gray-50 last:border-0"
              >
                <div>
                  <span className="font-semibold text-sm text-gray-900">{r.matched_name}</span>
                  <span className="text-xs text-gray-500 ml-2">{r.generic}</span>
                </div>
